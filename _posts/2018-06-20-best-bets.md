...
Find it in the [Admin Center](https://search.usa.gov/sites/) > YourSite > Manage Content > Best Bets: Text or Graphics
...
Create a comma-separated file with the following fields (in this order). Download our [sample template for uploading best bets](https://search.gov/files/best-bets-template-201603.csv) [CSV] to see the correct format.
...
We use [color coding](https://search.gov/manual/color-codes.html) to indicate each entry's status. 
...
***Did you know?*** Use the [Search Page Alert](https://search.gov/manual/system-alert.html) feature to add a text message to your search results page, which will appear at all times above all search results, *regardless of the query*.

***Did you know?*** Analyze the number of impressions and clicks and clickthru rate for each best bet on the [Monthly Reports](https://search.gov/manual/monthly-reports.html) page. Use the data to inform your titles, descriptions, and keywords and your decision to deactivate or delete an entry.

***Did you know?*** When you use the [sitelimit parameter](https://search.gov/manual/collections.html#sitelimit) to scope the search to a subsection of your website, we'll apply this sitelimit to your Best Bets so searchers see recommended pages from within that folder or subdomain only.
