...
2. Create a CNAME in your external DNS records for `search.youragency.gov`. Point it to `yoursitehandle.sites.infr.search.usa.gov`. Your site handle is listed on the [settings page](https://search.gov/manual/settings.html) in the Search Admin Center.
...
4. When your DNS record is in place and you have received confirmation that your domain mask has been added to our SSL certificate, change your search box's [form code](https://search.gov/manual/code.html) action from `search.usa.gov/search` to `search.youragency.gov/search`. 
